# Kartkówka: Tworzenie i obsługa formularzy (HTML + JS) - Grupa C

## Część 1: HTML (Struktura)

Stwórz w pliku HTML interfejs użytkownika zawierający następujące elementy:

1. Pole tekstowe na **Nazwę produktu**.
2. Pole tekstowe na **Kategorię produktu**.
3. Pole numeryczne na **Cenę produktu**.
4. Przycisk z napisem (np. "Dodaj produkt"), który po kliknięciu uruchomi odpowiednią funkcję w JavaScript.
...
`Produkt: <nazwa produktu>, kategoria: <kategoria>, kosztuje: <cena> zł`

> ⭐ **Zadanie dodatkowe (na ocenę celującą - 6):**
> Rozbuduj walidację (sprawdzanie) z punktu 2. Zanim skrypt wypisze wynik na stronie, sprawdź, czy wpisana cena jest logicznie poprawna, czyli czy jej wartość jest większa od 0 **oraz** mniejsza lub równa 10000. Jeśli cena nie mieści się w tym przedziale, skrypt nie powinien wyświetlać wyniku.
